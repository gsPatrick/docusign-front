...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-[240px] h-screen bg-[#F7F9FC] border-r shrink-0">
      
      {/* 
        Container de rolagem (Logo + Menu). Usa flex-grow para ocupar o espaço 
        entre o logo e o perfil.
        overflow-y-auto permite que esta seção role se o conteúdo (menu) for maior 
        que o espaço disponível.
      */}
      <div className="flex-grow flex flex-col p-4 space-y-8 overflow-y-auto">
        <div className="px-2 shrink-0"> {/* Logo é fixo, não rola */}
          <Link href="/dashboard" aria-label="Voltar para o Dashboard">
            <Image src="/logo.png" alt="Logo Doculink" width={140} height={32} priority />
          </Link>
        </div>
        {/* Navigation já usa flex-grow, mas o overflow-y-auto está no pai (esta div) */}
        <Navigation /> 
      </div>
